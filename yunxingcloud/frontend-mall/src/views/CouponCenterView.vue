...
async function claim(couponId: number) {
  ...
  try { await request.post(`/coupons/${couponId}/claim`); toast.success('领取成功'); load() } catch { toast.error('领取失败') }
  finally { claiming.value.delete(couponId) }
}

const discountLabel = (c: any) => c.type === 'full_reduction' ? `满${(c.threshold/100).toFixed(0)}减${(c.amount/100).toFixed(0)}` : `${c.discount}折`
...
<template>
  <div style="max-width:900px;margin:0 auto">
    <h2 style="font-size:20px;font-weight:700;margin-bottom:16px">🎫 优惠券中心</h2>
    ...
    <div v-else-if="activeTab==='available'" style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px">
      ...
      <div v-if="!availableCoupons.length" style="grid-column:1/-1;text-align:center;padding:60px;color:#999;background:#fff;border-radius:12px">
        <p style="font-size:48px;margin-bottom:12px">🎫</p><p>暂无可领取的优惠券</p>
      </div>
    </div>
    <div v-else-if="activeTab==='mine'" style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px">
      <div v-for="c in myCoupons" :key="c.id"
        ...
        <div style="flex:1;padding:16px">
          <div style="font-weight:600;font-size:14px;margin-bottom:4px">{{ c.name || '优惠券' }}</div>
          <div style="color:#999;font-size:11px">有效期至 {{ c.endTime?.substring(0,10) || '-' }}</div>
        </div>
      </div>
