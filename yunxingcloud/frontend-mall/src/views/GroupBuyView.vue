...
<template>
  <div style="max-width:900px;margin:0 auto">
    <div style="background:linear-gradient(135deg,#f10215,#ff6b6b);color:#fff;border-radius:16px;padding:32px;margin-bottom:24px;text-align:center;box-shadow:0 4px 20px rgba(228,57,60,.25)">
      ...
      <div v-for="g in groups" :key="g.id" @click="goProduct(g.productId)"
        ...
        <div style="padding:16px">
          ...
          <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:10px">
            <span style="color:#f10215;font-size:22px;font-weight:700">¥{{ (g.groupPrice/100).toFixed(2) }}</span>
            <span style="color:#999;font-size:12px;text-decoration:line-through">¥{{ (g.originalPrice/100).toFixed(2) }}</span>
            <span style="color:#f10215;font-size:11px;margin-left:auto">{{ g.currentMembers || 0 }}/{{ g.minMembers }}人</span>
          </div>
          <div style="background:#f5f5f5;border-radius:4px;height:6px;overflow:hidden">
            <div style="height:100%;background:linear-gradient(90deg,#f10215,#ff6b6b);border-radius:4px;transition:width .6s" :style="{width:progress(g)+'%'}"></div>
          </div>
          <button style="width:100%;margin-top:12px;padding:8px;background:#f10215;color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px;font-weight:600"
