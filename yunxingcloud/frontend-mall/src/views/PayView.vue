    ...
    <div style="background:#fff;border-radius:12px;padding:32px;box-shadow:0 2px 12px rgba(0,0,0,.06)">
      ...
      <template v-else>
        ...
        <div v-if="order" style="color:#999;font-size:13px;margin-bottom:8px">
          订单 {{ order.orderNo }} · 应付 <b style="color:#f10215;font-size:18px">¥{{ (order.totalAmount/100).toFixed(2) }}</b>
        </div>
        ...
        <div style="margin-bottom:24px">
          <div @click="selectedChannel='wechat'" style="display:flex;align-items:center;padding:16px;border-radius:8px;margin-bottom:12px;cursor:pointer;transition:all .2s"
               :style="{border:selectedChannel==='wechat'?'2px solid #f10215':'1px solid #eee',background:selectedChannel==='wechat'?'#fff5f5':'#fff'}">
            ...
            <div style="width:18px;height:18px;border-radius:50%;border:2px solid #f10215;display:flex;align-items:center;justify-content:center"
                 :style="{background:selectedChannel==='wechat'?'#f10215':''}"><span v-if="selectedChannel==='wechat'" style="color:#fff;font-size:11px">✓</span></div>
          </div>
          <div @click="selectedChannel='alipay'" style="display:flex;align-items:center;padding:16px;border-radius:8px;cursor:pointer;transition:all .2s"
               :style="{border:selectedChannel==='alipay'?'2px solid #f10215':'1px solid #eee',background:selectedChannel==='alipay'?'#fff5f5':'#fff'}">
            ...
            <div style="width:18px;height:18px;border-radius:50%;border:2px solid #f10215;display:flex;align-items:center;justify-content:center"
                 :style="{background:selectedChannel==='alipay'?'#f10215':''}"><span v-if="selectedChannel==='alipay'" style="color:#fff;font-size:11px">✓</span></div>
          </div>
        </div>
        <button @click="confirmPay" :disabled="paying"
                style="width:100%;height:48px;background:#f10215;color:#fff;border:none;border-radius:8px;font-size:16px;cursor:pointer;font-weight:700;transition:background .2s"
                @mouseenter="(e:any) => e.target.style.background='#d4000f'" @mouseleave="(e:any) => e.target.style.background='#f10215'">
