...
const columns: DataTableColumns<any> = [
  ...
  },
  {
    ...
    render: (r: any) => h(NSpace, { size: 'small' }, { default: () => [
      h(NButton, { size: 'tiny', onClick: () => { editingId.value = r.id; form.value = { name: r.name, icon: r.icon || '', sortOrder: r.sortOrder || 0, status: r.status, parentId: r.parentId || null }; showModal.value = true } }, { default: () => '编辑' }),
...
async function del(id: number) { try { await request.delete(`/api/categories/${id}`); notify.success('已删除'); load() } catch { notify.error('删除失败') } }
