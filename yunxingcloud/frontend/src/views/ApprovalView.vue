...
async function loadItems() { loading.value=true;try{const r=await (tab.value==='my'?fetchMyApprovals():fetchPendingApprovals());items.value=r.data;pending.value=r.data}catch { notify.error(t('common.loadFail')) }loading.value=false }
...
async function submit(){saving.value=true;try{await createApproval(form.value);showModal.value=false;form.value={approver:'admin',type:t('approval.typeLeave'),title:'',content:''};loadItems();notify.success(t('approval.createSuccess'))}catch { notify.error(t('common.saveFailed')) }finally{saving.value=false}}
