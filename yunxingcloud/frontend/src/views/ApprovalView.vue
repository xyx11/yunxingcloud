...
async function approve(id:number){await request.put(`/api/approval/${id}/approve`,{});loadItems();notify.success(t('approval.approveSuccess'))}
async function reject(id:number){await request.put(`/api/approval/${id}/reject`,{});loadItems();notify.success(t('approval.rejectSuccess'))}
async function submit(){saving.value=true;try{await request.post('/api/approval',form.value);showModal.value=false;form.value={approver:'admin',type:t('approval.typeLeave'),title:'',content:''};loadItems();notify.success(t('approval.createSuccess'))}catch{}finally{saving.value=false}}
