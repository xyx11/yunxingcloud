...
const form = ref({ name:'', contact:'', phone:'', email:'', address:'', status:'1' })
...
const columns: DataTableColumns<Record<string, unknown>> = [
  ...
  { title: '状态', key: 'status', width: 70, render(r: Record<string, unknown>) { return h(NTag, { size: 'small', type: r.status === '1' ? 'success' : 'default' }, { default: () => r.status === '1' ? '启用' : '停用' }) } },
  { title: '商品数', key: 'productCount', width: 70, render(r: Record<string, unknown>) { return h(NTag, { size: 'small', type: 'info' }, { default: () => String(r.productCount || 0) }) } },
  { title: t('common.actions'), key:'act', width:120, render(r: Record<string, unknown>){ return h(NSpace,{size:'small'},{default:()=>[
    h(NButton,{size:'tiny',onClick:()=>{editingId.value=r.id as number;form.value={name:r.name as string,contact:(r.contact as string)||'',phone:(r.phone as string)||'',email:(r.email as string)||'',address:(r.address as string)||'',status:(r.status as string)||'1'};showModal.value=true}},{default:()=>'编辑'}),
...
function add() { editingId.value=null; form.value={name:'',contact:'',phone:'',email:'',address:'',status:'1'}; showModal.value=true }
onMounted(load)
</script>
<template>
  <n-card title="供应商管理">
    ...
    <n-drawer v-model:show="showModal" :width="420" placement="right">
      <n-drawer-content :title="editingId?'编辑供应商':'新增供应商'" closable>
        ...
        <n-form :model="form" label-placement="left" label-width="70" size="small">
          ...
          <n-form-item label="状态"><n-select v-model:value="form.status" :options="[{label:'启用',value:'1'},{label:'停用',value:'0'}]" /></n-form-item>
        </n-form>
      </n-drawer-content>
    </n-drawer>
  </n-card>
